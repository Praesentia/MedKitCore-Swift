...
public class NetPortFactory: PortFactory {
     ...
     */
    public func instantiateConnection(to device: DeviceBackend, using principalManager: PrincipalManager) -> ClientConnection?
    {
        var connection: ClientConnection?
        
        if let port = instantiatePort() {
            connection = type.clientFactory.instantiate(to: port, for: device, using: principalManager)
        }
